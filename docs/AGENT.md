...
**큰 틀 순서:** 퍼블리싱 → 서버(API/Server Action) → 프론트(연동) → 테스트 → 커밋

**작은 틀 (커밋 1단위):**

1. **기능 구현** — 코드 작성
2. **테스트 방법 안내** — Vitest 단위 테스트 작성/실행 명령어 (사용자가 직접 실행)
3. **화면 테스트 항목 안내** — 실제 화면에서 확인할 항목 리스트 (예: "로그인 클릭 → 구글 창 뜨는지", "저장 후 새로고침해도 데이터 남는지")
4. **문제 확인** — 사용자가 테스트 후 결과 알림
5. **문제 있으면 수정** / **없으면 커밋명 추천**
6. 커밋 후 다음 단위로

### PR 후 MD 업데이트 정책

- PR 머지 후 **꼭 필요한 경우에만** MD 업데이트
- 업데이트 대상:
  - 설계와 다르게 구현된 부분 (실제 동작이 truth)
  - 추가/변경된 기술적 결정
  - 새로 발견한 제약사항
- **업데이트 안 함:**
  - 이미 적힌 거 그대로 구현된 경우
  - 단순 코드 변경, 버그 수정
- 원칙: **MD가 길어지는 건 지양**. 필요한 것만 짧게 반영.
...
- `main` — 배포 (운영)
- `develop` — 기본 브랜치 (통합)
- `feat/이슈번호-작업명` — 기능 브랜치 (develop에서 따고 develop으로 PR)
...
1. **AI가 이슈 제목 + 간단한 내용 추천** (작업 단위마다)
2. 사용자가 GitHub에 이슈 등록 → 이슈 번호 받기
3. `develop`에서 브랜치 따기: `feat/12-login-page`
4. 작업 + 커밋 (커밋 단위로 잘게)
5. push → PR (`develop` 대상)
6. PR 본문에 `Closes #12` 적기 → 머지 시 이슈 자동 닫힘
7. 머지 후 브랜치 자동 삭제
...
> 위 구조는 시작 가이드. 실제 폴더는 만들면서 자연스럽게 채워나감. 변경/추가된 부분은 PR 후 MD에 반영.
