...
interface EditorState {
  ...
  savedSnapshot: string // 마지막 저장본 직렬화로 변경 감지 기준
  initialSnapshot: string // 처음 로드 시 직렬화로 idle 판단 기준
    ...
    devtools(
      (set) => ({
        ...
        setTitle: (title) =>
          set((s) => ({
            title,
            ...dirtyFrom(title, s.sections, s.savedSnapshot, s.initialSnapshot),
          })),
            ...
            const sections = s.sections.map((section) => {
              ...
              if (next.type === 'image' && next.content.src) {
                ...
                next = {
                  ...
                  content: {
                    ...next.content,
                    images: next.content.images.map((image) => ({
                      ...image,
                      url: swap(image.url),
                    })),
                  },
        ...
        addColumnChild: (sectionId, colIndex, type) =>
          set((s) => {
            ...
            const sections = s.sections.map((section) => {
              ...
              const columns = section.content.columns.map((col, i) =>
                i === colIndex ? [child] : col,
              )
              return { ...section, content: { ...section.content, columns } }
            })
            return {
          ...
          set((s) => {
            const sections = s.sections.map((section) => {
              ...
              return {
                ...
                content: {
                  ...section.content,
                  columns: moveItem(section.content.columns, from, to),
                },
      ...
      { name: `editor:${initial.pageId}`, enabled: process.env.NODE_ENV === 'development' },
    ),
  )
}
